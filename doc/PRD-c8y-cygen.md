...
- **Model is a config field.** Start on **`claude-sonnet-5`** (adaptive thinking, `effort: high`); escalate to **`claude-opus-4-8`** via a one-line config switch if quality requires. Tune cost/quality empirically.
